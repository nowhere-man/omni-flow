...
- 修改 Android 的 UI 或交互时，先判断该行为是否依赖平台专属能力。
- 共享模块的行为变更必须在同一次变更中同步验证 Android 客户端。
...
- 在客户端之间复用通用模型和服务。不要在客户端代码中重新实现 `shared` 中已有的规则。
- 共享行为不要求呈现完全相同。保留平台原生的容器和交互模式。
...
- 在相关情况下检查浅色/深色模式、每种主题色、选中/未选中状态、空/有数据/错误状态、导航、文本截断和小屏幕布局。
...
- 确认最终 Release 包含预期的 Android APK。
